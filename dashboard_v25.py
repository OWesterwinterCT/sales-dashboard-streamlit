...
def load_items():
    ...
    # revenue
    df["Revenue"] = df["NetPrice"] * df["QtyOrdered"]

    # ✅ FIX: parse OrderDate
    df["OrderDate"] = pd.to_datetime(df["OrderDate"], errors="coerce", utc=True)
    df["OrderDate"] = df["OrderDate"].dt.tz_localize(None)

    # ✅ align with dashboard time logic
    df["Month"] = df["OrderDate"].dt.to_period("M").dt.to_timestamp()
    df["Week"] = df["OrderDate"].dt.to_period("W").dt.to_timestamp()
...
date_range = st.sidebar.date_input(
    "Date range",
    [orders_df["OrderDate"].min(), orders_df["OrderDate"].max()]
)
...
receipts_df = receipts_df[
    (receipts_df["ReceiptDate"] >= pd.to_datetime(date_range[0])) &
    (receipts_df["ReceiptDate"] <= pd.to_datetime(date_range[1]))
]
...
if zoom_option == "Last 12 Months":
    ...
    receipts_df = receipts_df[
        receipts_df["ReceiptDate"] >= latest_date - pd.DateOffset(months=12)
    ]
...
if zoom_option == "Last 26 Weeks":
    ...
    receipts_df = receipts_df[
        receipts_df["ReceiptDate"] >= latest_date - pd.DateOffset(weeks=26)
    ]
...
sales_tab, products_tab, purchase_tab, profit_tab = st.tabs(
    ["Sales Dashboard", "Products Dashboard", "Purchasing Dashboard", "Profitability Dashboard"]
)
...
with sales_tab:
    ...
    revenue_ts = orders_df.groupby(time_col)["Amount"].sum().reset_index().sort_values(time_col)
    orders_ts = orders_df.groupby(time_col)["OrderNumber"].nunique().reset_index().sort_values(time_col)

    monthly_sales = orders_df.groupby("Month")["Amount"].sum().reset_index()
    weekly_sales = orders_df.groupby("Week")["Amount"].sum().reset_index()
    ...
    col4.metric("Avg Monthly Revenue", f"${avg_monthly_revenue:,.0f}", f"{monthly_change:+.1f}%")
    col5.metric("Avg Weekly Revenue", f"${avg_weekly_revenue:,.0f}", f"{weekly_change:+.1f}%")
    col6.metric("Avg Revenue per Order", f"${avg_revenue_per_order:,.0f}")

    st.divider()

    fig = px.line(revenue_ts, x=time_col, y="Amount", markers=True)
    ...
    summary_table = (
        ...
        .agg(Orders=("OrderNumber", "nunique"), Revenue=("Amount", "sum"))
        .reset_index()
    ...
    fig_top = px.bar(customer_rev.head(10), x="Amount", y="CustomerName", orientation="h")
    st.plotly_chart(fig_top, width="stretch")

    fig_states = px.pie(state_counts, names="State", values="Customers")
    ...
    fig_pareto = px.bar(pareto.head(20), x="CustomerName", y="Amount")
    ...
    agent_table = (
        ...
        .agg(Revenue=("Amount", "sum"), Orders=("OrderNumber", "nunique"))
        .reset_index()
    )
    ...
    fig_forecast = px.line(forecast_df, x=time_col, y="Amount")
    fig_forecast.add_scatter(x=future_df[time_col], y=future_df["Forecast"], mode="lines+markers")
    ...
    fig_orders = px.line(orders_forecast, x=time_col, y="OrderNumber")
    fig_orders.add_scatter(x=future_orders_df[time_col], y=future_orders_df["ForecastOrders"], mode="lines+markers")
...
with products_tab:
    ...
    avg_items_per_order = items_df["QtyOrdered"].sum() / orders_df["OrderNumber"].nunique()
    avg_item_value = (items_df["NetPrice"] * items_df["QtyOrdered"]).mean()

    col1, col2 = st.columns(2)
    col1.metric("Avg Items per Order", f"{avg_items_per_order:,.2f}")
    col2.metric("Avg Item Revenue", f"${avg_item_value:,.2f}")

    st.divider()

    top_qty = (
        items_df.groupby("ProductName")["QtyOrdered"]
        .sum()
        .sort_values(ascending=False)
        .head(15)
        .reset_index()
    ...
    top_price = (
        items_df.groupby("ProductName")["NetPrice"]
        .mean()
        .sort_values(ascending=False)
        .head(15)
        .reset_index()
    ...
    top_rev = (
        items_df.groupby("ProductName")["Revenue"]
        .sum()
        .sort_values(ascending=False)
        .head(15)
        .reset_index()
    ...
    top_products = top_qty["ProductName"].tolist()

    items_time = items_df[items_df["ProductName"].isin(top_products)].copy()

    items_time[time_col] = items_time["OrderDate"].dt.to_period(
        "M" if time_unit == "Month" else "W"
    ).dt.to_timestamp()

    ts = (
        items_time.groupby([time_col, "ProductName"])["QtyOrdered"]
        .sum()
        .reset_index()
    )
